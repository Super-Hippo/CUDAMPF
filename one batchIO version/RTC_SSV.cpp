...
void RTC_SSV(unsigned int number, const char* GPU_kernel, HMMER_PROFILE *hmm,
...
{	
	...
	checkCudaErrors(cuMemcpyHtoD(mat_m, hmm->ssv_vec, hmm->msvQ * PROTEIN_TYPE * sizeof(__32uint__)));
   ...
   // printf("%d,%d,%d,%d,%f\n",hmm->base_b,hmm->bias_b,hmm->tbm_b,hmm->tec_b,hmm->scale_b);

    /* parameters for kernel funciton */
	void *arr[] = { &d_seq, &number, &d_offset,
					&score, &d_len, &d_len_6r, &mat_m, 
					&(hmm->base_b), &(hmm->bias_b), &(hmm->tbm_b), &(hmm->tec_b),
					&(hmm->scale_b)};
    ...
    unsigned long n_recal_msv = 0;			/* # of seqs need be re-calculated in MSVFilter */
	unsigned long n_pass_ssv = 0;			/* # of seqs pass SSVFilter and go further into VITFilter */
	unsigned long n_pass_vit = 0;			/* # of seqs pass SSVFilter and VITFilter and go further into Forward/BackwardFilter */

	float 	nullsc = 0.0f;
	double  seq_score = 0.0f;				/* Final score of each seq for obtain P-values   */
	double 	P_value = 0.0f;					/* Used to filter seqs                           */

	for (int i = 0; i < number; i++)
	{
		//printf("%d:%d\n",i, (int)pVal[i]);
		/* count re-calculated seqs */
		if ((int)pVal[i] == 0) {
			n_recal_msv++;
			continue;
		}
		...
		/* P-value */
		NullOne(seq_len[i], &nullsc);
		seq_score = (pVal[i] - (double)nullsc) / eslCONST_LOG2;
		P_value = esl_gumbel_surv(seq_score, hmm->MU[0], hmm->LAMBDA[0]);

		/* count seqs pass SSV and VIT */
		if (P_value <= F2)
			n_pass_vit++;

		if ((P_value <= F1) && (P_value > F2))
			n_pass_ssv++;
	}

	printf("|					Output of SSVFilter					|\n");
